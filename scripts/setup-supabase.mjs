...
async function setupDatabase() {
  const client = new Client({
    host: 'db.hocjetqkgrptxdbsmmgx.supabase.co',
    port: 5432,
    database: 'postgres',
    user: 'postgres',
    password: 'beginHabit1_1',
...
DROP POLICY IF EXISTS "anon_all_quiz_history" ON quiz_history;
DROP POLICY IF EXISTS "anon_all_student_badges" ON student_badges;
DROP POLICY IF EXISTS "anon_all_clipboard_entries" ON clipboard_entries;
...
`;
    ...
    const tables = await client.query(`
      ...
      WHERE table_schema = 'public' AND table_name IN ('students','daily_entries','quiz_history','student_badges','clipboard_entries')
      ORDER BY table_name
    `);
